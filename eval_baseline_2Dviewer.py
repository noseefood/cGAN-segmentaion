    ...
    def start(self):
        ...
        if Video_recording:
            ...
            for video_name, video_dir in self.video_dirs.items():
                ...
                setattr(self, f"videoWriter_{video_name}", cv2.VideoWriter(video_dir, fourcc, fps, img_size, isColor=isColor))
        ...
        metrics = ['Recall', 'Precision', 'F2', 'dice', 'iou', 'Continuity', 'TipError', 'Angle', 'centerError']
        ...
        for i, (input, target) in enumerate(zip(self.imgs, self.masks)):
            ...
            for model_name, model in models.items():
                ...
                TipError, AngleError, centerError, filter_output = self.Calculate_Error_object.Calculate_TipError(pred=models_metrics[model_name]['output'], mask=true_mask, method="canny")
                
                models_metrics[model_name]['TipError'].append(TipError)
                models_metrics[model_name]['Angle'].append(AngleError)
                models_metrics[model_name]['centerError'].append(centerError)
                
                if colinear_filter:
                    # Using colinear filter result to replace the original output for visualization
                    models_metrics[model_name]['output'] = filter_output

            if visualizaion:
                for model_name in models.keys():
                    if models_metrics[model_name]['output'] is not None and models_metrics[model_name]['output'].size > 0:
                        cv2.imshow(f"output_{model_name}"+self.colinear_used, models_metrics[model_name]['output'])
                    if true_mask is not None and true_mask.size > 0:
                        cv2.imshow(f"true_mask", true_mask)
...
if __name__ == "__main__":
    test_mode = "Compounding" # "Insertion" or "Compounding"
    dataset = 5 # 3,4,6 for insertion mode; 1,5 for compounding mode
