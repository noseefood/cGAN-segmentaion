...
net_4 = NetworkInference_Unet(method="AttentionUnet")
net_5 = NetworkInference_UnetPLUSPLUS()

# time_start = time.time()
# output = net.inference(img)
# time_end = time.time()
# print("DeepLabV3PLUS: ", time_end - time_start)

# time_start = time.time()
# output = net_2.inference(img)
# time_end = time.time()
# print("GANVer2: ", time_end - time_start)

# time_start = time.time()
# output = net_3.inference(img)
# time_end = time.time()
# print("Unet: ", time_end - time_start)

# time_start = time.time()
# output = net_4.inference(img)
# time_end = time.time()
# print("AttentionUnet: ", time_end - time_start)

# time_start = time.time()
# output = net_5.inference(img)
# time_end = time.time()
# print("Unet++: ", time_end - time_start)

# Create a dictionary of networks
networks = {'GANVer2': net_2, 'Unet': net_3, 'AttentionUnet': net_4, 'Unet++': net_5, 'DeepLabV3PLUS': net}
# 注意这里第一个计算的不准确,需要计算两次换一下第一个的顺序
# Create a dictionary to store the running times
running_times = {name: [] for name in networks}

# Run each network 5 times
for _ in range(5):
    for name, network in networks.items():
        time_start = time.time()
        output = network.inference(img)
        time_end = time.time()
        running_times[name].append(time_end - time_start)

# Calculate the average and std of the running times
for name, times in running_times.items():
    avg_time = np.mean(times)
    std_time = np.std(times)
    print(f"{name}: avg = {avg_time}, std = {std_time}")
